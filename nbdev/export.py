...
def reset_nbdev_module():
    ...
    with open(fname, 'w') as f:
        f.write("# AUTOGENERATED BY NBDEV! DO NOT EDIT!")
...
def split_flags_and_code(cell, return_type=list):
    ...
    return res if return_type is list else tuple('\n'.join(r) for r in res)
...
def _notebook2script(fname, modules, silent=False, to_dict=None, bare=False):
    ...
    for i,c,(e,a) in cells:
        ...
        if a and to_dict is None:
            _add2all(fname_out, [f"'{f}'" for f in names if '.' not in f and len(f) > 0] + extra)
...
def update_baseurl():
    ...
    with open(fname, 'r') as f: code = f.read()
    if _re_baseurl.search(code) is None:
...
def nbglob(fname=None, recursive=None, extension='.ipynb', config_key='nbs_path') -> L:
    ...
    if str(fname).endswith('**'):
        fname,pat = fname[:-2], f'**/{pat}'
